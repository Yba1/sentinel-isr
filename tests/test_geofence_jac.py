"""Phase 3.5 acceptance tests for the JAC half of geofence intrusion.

Scope note: this file tests ``jac/geofence.jac`` -- the transition tracking, the
event log and the compound severity rules. The point-in-polygon geometry in
``tracker/geofence.py`` is a separate module with its own test file; here it is
used as a fixture, not as the thing under test. If a test in this file fails, the
bug is in the Jac.

Everything is marked ``@pytest.mark.jac`` because it needs jaclang on a Python
3.12 interpreter; deselect with ``-m "not jac"``. The module is driven in-process
via jaclang's meta importer (``import jaclang`` then ``import geofence``), so the
assertions read live graph state and the real event dicts rather than scraped
stdout.

All coordinates are LOCAL ENU METRES. Never lon/lat.
"""
...
def gf():
    """The compiled jac/geofence.jac, imported as a Python module.

    Both skips matter. Without the jaclang skip this file explodes on the
    anaconda 3.11 interpreter; without the tracker.geofence skip a missing
    geometry module turns the whole suite into collection errors instead of
    skips.
    """
    pytest.importorskip("jaclang", reason="jaclang (Python 3.12+) not installed")
    pytest.importorskip("tracker.geofence", reason="tracker.geofence not present")
    import jaclang  # noqa: F401  -- installs the .jac meta importer

    if _JAC_DIR not in sys.path:
        sys.path.insert(0, _JAC_DIR)
    import geofence as jac_geofence
...
def mon(gf, fences):
    ...
    whole pytest process and ``root`` is persisted to .jac/data on disk between
    runs, so without it every test would see its predecessors' events.
    """
    gf.set_index(fences)
    gf.reset()
    return gf
